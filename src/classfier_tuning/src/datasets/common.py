...
def get_features(is_train, image_encoder, dataset, device, is_real=True, args=None):
    split = 'train' if is_train else 'val'
    if is_real is False:
        split= args.cache_name_syn
...
class FeatureDataset(Dataset):
    def __init__(self, is_train, image_encoder, dataset, device, is_real=True, args=None):
        self.data = get_features(is_train, image_encoder, dataset, device, is_real, args=args)
...
def get_dataloader(dataset, is_train, args, image_encoder=None, is_real=True):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(
            is_train, image_encoder, dataset, args.device, is_real, args=args)
