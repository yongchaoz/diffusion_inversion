...
class CLIP_selector(nn.Module):
    def __init__(self, clip_model, train_preprocess, val_preprocess, feature_cache='imagenet_text_feature.pt'):
        super().__init__()
...
def main(input_path, output_path, num_group, group_id, threshold=0.1, min_shot=1):
    ...
    clip_selector = CLIP_selector(model, preprocess, preprocess, feature_cache='imagenet_text_feature.pt')
    ...
    for cls_name in tqdm(train_cls):
        tmp_dir = os.path.join(output_path, cls_name)
    
        if not os.path.exists(tmp_dir):
            os.makedirs(tmp_dir, exist_ok=True)

        for g in os.listdir(f'{input_path}/{cls_name}'):
            ...
            if not os.path.exists(os.path.join(tmp_dir, g)) or len(os.listdir(os.path.join(tmp_dir, g)))==0:
                ...
                print('{}: selected {}/{} images'.format(cls_name, len(selected_ims), len(cls_ims)))
                
                for ims in tqdm(selected_ims): 
                    split = ims.split('/')  
